...
module.exports.handleEvent = async function({ api, event, args, Threads, Users }) {
  ...
  if ((event.body.toLowerCase() == "good morning") || (event.body.toLowerCase() == "gm")) {
   ...
   if ((event.body.toLowerCase() == "@jerry nobi") || (event.body.toLowerCase() == "Jerry")) {
     return api.sendMessage("JERRY SIRF Mera HAI 😍<3", threadID);
   };
   ...
   if ((event.body.toLowerCase() == "kutti bot") || (event.body.toLowerCase() == "kutti")) {
     return api.sendMessage("️Bhow bhow jerry Malik 😍😘 :))))", threadID);
   };

   if ((event.body.toLowerCase() == "khushi+aryan") || (event.body.toLowerCase() == "aryan")) {
     return api.sendMessage("️aryan sirf khushi ka hai 😍 :))))", threadID);
   };
